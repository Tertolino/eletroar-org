...
export default function EditarOrdem() {
  ...
  const { servicos, updateServico, tecnicos } = useServicosStore();
  ...
  useEffect(() => {
    if (servicoExistente) {
      setCliente(servicoExistente.cliente);
      setEndereco(servicoExistente.endereco);
      setContato(servicoExistente.contato);
      setData(servicoExistente.data);
      setDesconto(servicoExistente.desconto);
      setFormaPagamento(servicoExistente.formaPagamento);
      setObservacoes(servicoExistente.observacoes);
      setServicosItems(servicoExistente.servicos);
      setTecnicosServico(servicoExistente.tecnicos);
    }
  }, [servicoExistente]);
  ...
  const removeServicoItem = (itemId: string) => {
    if (servicosItems.length > 1) {
      setServicosItems(servicosItems.filter((s) => s.id !== itemId));
    }
  };

  const updateServicoItem = (itemId: string, field: keyof ServicoItem, value: string | number) => {
    setServicosItems(
      servicosItems.map((s) => {
  ...
  const removeTecnicoServico = (tecId: string) => {
    if (tecnicosServico.length > 1) {
      setTecnicosServico(tecnicosServico.filter((t) => t.id !== tecId));
    }
  };

  const updateTecnicoServico = (tecId: string, field: keyof TecnicoServico, value: string | number | boolean) => {
    setTecnicosServico(
  ...
  const validate = (): boolean => {
    ...
    const hasValidTecnico = tecnicosServico.some((t) => t.tecnicoId);
    if (!hasValidTecnico) {
      newErrors.tecnicos = "Selecione pelo menos um técnico";
    }
    
    const hasValidServico = servicosItems.some((s) => s.tipo && s.quantidade > 0);
    if (!hasValidServico) {
      newErrors.servicos = "Adicione pelo menos um serviço válido";
    }
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        >
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Altere os dados da ordem</p>
          </div>
        </motion.div>
